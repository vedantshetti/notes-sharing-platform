  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq('year_id', 3) // Fetch only 3rd-year subjects
        .order('subject_name', { ascending: true }); // Optional: Order alphabetically
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {subjects.map((subject, index) => (
          <Link
            to={`/thirdyear/subject/${encodeURIComponent(subject.subject_name)}`}
            key={index}
...
export default ThirdYearSubjectList;
