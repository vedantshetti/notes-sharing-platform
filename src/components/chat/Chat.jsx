...
const Chat = () => {
  ...
  const [sendButtonDisabled, setSendButtonDisabled] = useState(false); // Track send button disable state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Disable the send button for 5 seconds after sending a message
      setSendButtonDisabled(true);
      setTimeout(() => {
        setSendButtonDisabled(false); // Re-enable the send button after 5 seconds
      }, 5000);
    } catch (error) {
      setError(error.message);
    }
  ...
  return (
    <div className="flex h-full min-h-screen bg-gray-100">
      ...
      <div className="flex flex-col justify-between flex-1 w-full lg:w-3/4 p-4 lg:p-8">
        ...
        {/* Message input form */}
        <form onSubmit={handleSubmit} className="mt0 lg:mt-1 flex flex-col sm:flex-row gap-2 items-center">
  <input
    type="text"
    value={newMessage}
    ...
    disabled={!currentUser || sendButtonDisabled} // Disable the input if button is disabled
  />
  <input
    type="file"
    onChange={(e) => setSelectedFile(e.target.files[0])}
    className="px-4 py-2 border rounded-lg shadow-md"
    disabled={sendButtonDisabled} // Disable the file input if button is disabled
  />
  <button
    type="submit"
    disabled={!currentUser || (!newMessage.trim() && !selectedFile) || sendButtonDisabled} // Disable the button if the timer is active
