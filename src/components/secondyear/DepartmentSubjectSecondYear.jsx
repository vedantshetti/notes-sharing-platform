...
const DepartmentSubjectSecondYear = () => {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      ...
      try {
        ...
        // Step 2: Fetch subjects based on departmentId and year (second year)
        const { data: subjectsData, error: subjectsError } = await supabase
          .from("subjects")
          .select("subject_name")
          .eq("department_id", departmentId) // Filter by department ID
          .eq("year_id", 2); // Filter by 2nd year
        ...
        {notes.length > 0 ? (
          notes.map((subject) => (
            <Link
              ...
              to={`/2nd-year/${departmentName}/${encodeURIComponent(subject.subject_name)}`} // Encode subject name for proper URL
...
export default DepartmentSubjectSecondYear;
